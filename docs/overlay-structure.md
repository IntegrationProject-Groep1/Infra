...
**Why we did not name the dev secret `shift-secrets-dev`:**

If the dev overlay generated a Secret named `shift-secrets-dev`, every single Deployment in `base/` that references `name: shift-secrets` would need a dev-specific patch to reference `shift-secrets-dev` instead. That would require 18+ patches across the codebase for no real benefit. Namespace isolation already makes the two secrets completely independent.

### Secret file locations

| File | Purpose | Committed to git? |
|------|---------|------------------|
| `overlays/prod/.env` | Real production credentials | **NO** — gitignored |
| `overlays/dev/.env` | Real development credentials | **NO** — gitignored |
| `overlays/prod/.env.example` | Template with all keys, empty values | YES |
| `overlays/dev/.env.example` | Template with all keys, empty values | YES |

The `.env` files must exist on the VM before a deploy runs. They are placed there manually by the infra team and are never committed.

---

## 9. Keel (image updater) placement

### What Keel does

Keel watches Kubernetes Deployments for image tag updates. When a new image is pushed to a container registry, Keel automatically updates the matching Deployment to use the new image. This means teams can deploy to production simply by pushing a new image — no manual `kubectl set image` needed.

Keel is configured to watch only the `shift-festival` namespace via the `NAMESPACE` environment variable in `keel/keel.yaml`. This was changed from `""` (all namespaces) to `"shift-festival"` in this PR. Dev is deliberately excluded from auto-updates.

### Why Keel is not inside the overlay

Keel runs in its own `keel` Kubernetes namespace. If we added `../../keel` inside `overlays/prod/kustomization.yaml`, the overlay's `namespace: shift-festival` transformer would override Keel's namespace from `keel` to `shift-festival`. This would break Keel completely — its ServiceAccount, ClusterRoleBinding, and Service would all land in the wrong namespace.

### How it is included instead

Keel is referenced from the **root `kustomization.yaml`**, which lives outside any overlay and applies no namespace transformer:
...
When Kustomize processes this, `overlays/prod` is rendered with its own namespace transformer (only affecting base resources), and `keel/` is rendered without any transformer (preserving `namespace: keel` throughout).

This root file is a **temporary passthrough** until Kainy updates the CI/CD pipeline to call each overlay directly. After that, the root `kustomization.yaml` can be removed.
...
```bash
# Roll back a specific Deployment to the previous version
kubectl rollout undo deployment/<name> -n shift-festival

# Check rollout history (shows all previous versions)
...
The `secretGenerator` did not run — usually because the `.env` file was missing when `kubectl apply` was called. Ensure `overlays/prod/.env` (or `overlays/dev/.env`) exists before running apply.
